...
experts (if I can be called an expert). For example, inserting `rowwise()` into
a pipe can break existing `mutate()` calls downstream in a way that
...
deal with managing variables (similar to how `rowwise()` works).
